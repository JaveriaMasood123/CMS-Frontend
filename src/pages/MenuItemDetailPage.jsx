...
//     const { itemId } = ReactRouterDOM.useParams();
...
//                 <ReactRouterDOM.Link to="/menu" className="mt-4 inline-block bg-teal-600 text-white px-6 py-2 rounded-md hover:bg-teal-700">
//                     Back to Menu
//                 </ReactRouterDOM.Link>
//             </div>
//         );
...
//                         Provided by {caterer?.businessName || 'GourmetGo'}
//                     </p>
...
const MenuItemDetailPage = () => {
    ...
    if (!item) {
        return (
            <div className="text-center py-10">
                <h1 className="text-2xl font-bold" style={{ color: '#030303' }}>Menu Item Not Found</h1>
                <ReactRouterDOM.Link 
                    to="/menu" 
    ...
    return (
        <div className="p-8 rounded-lg shadow-lg max-w-6xl mx-auto border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
            <div className="grid md:grid-cols-2 gap-12">
                ...
                <div className="flex flex-col justify-center">
                    <p className="text-sm font-semibold uppercase tracking-wider mb-2" style={{ color: '#666666' }}>
                        Provided by {caterer?.businessName || 'GourmetGo'}
                    </p>
                    ...
                    <div className="flex gap-4">
                        ...
                        {currentUser?.role === UserRole.CUSTOMER && (
                            <button
                                onClick={() => item && toggleFavorite(item.id)}
                                className="flex-shrink-0 border p-3 rounded-md font-semibold text-lg hover:opacity-90 transition-opacity"
                                style={{ 
                                    backgroundColor: isItemFavorite ? '#123458' : 'transparent', 
                                    borderColor: '#123458', 
                                    color: isItemFavorite ? 'white' : '#123458' 
                                }}
                                aria-label={isItemFavorite ? `Remove ${item.name} from favorites` : `Add ${item.name} to favorites`}
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill={isItemFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={1}>
            ...
            {relatedItems.length > 0 && (
                <div className="mt-12 pt-8 border-t" style={{ borderColor: '#E5E7EB' }}>
                    <h2 className="text-2xl font-bold mb-6" style={{ color: '#030303' }}>You Might Also Like</h2>
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {relatedItems.map(relatedItem => (
                            <MenuItemCard key={relatedItem.id} item={relatedItem} />
                        ))}
