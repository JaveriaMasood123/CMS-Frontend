...
const StatCard = ({ title, value, icon, colorClass }) => (
    <div className={`p-6 rounded-lg shadow-md border-2 ${colorClass}`} style={{ borderColor: '#123458' }}>
        <div className="flex items-center">
            <div className="mr-4">{icon}</div>
            <div>
                <h3 className="text-lg font-semibold" style={{ color: '#030303' }}>{title}</h3>
                <p className="text-4xl font-bold" style={{ color: '#030303' }}>{value}</p>
            </div>
        </div>
    </div>
);
...
const CatererDashboardHome = () => {
    ...
    const RevenueIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#123458' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01" /></svg>;
    const MenuIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#123458' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>;
    const PendingIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#123458' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;

    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-2xl font-bold mb-2" style={{ color: '#030303' }}>Welcome, {currentUser?.businessName}!</h1>
                <p style={{ color: '#666666' }}>Here's a quick look at your business on GourmetGo.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-lg shadow-md border-2" style={{ borderColor: '#123458' }}>
                    <h2 className="text-xl font-bold mb-4" style={{ color: '#030303' }}>Recent Orders</h2>
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead style={{ backgroundColor: '#F8FAFC' }}>
                                <tr>
                                    <th className="text-left py-2 px-3 font-medium" style={{ color: '#030303' }}>Order ID</th>
                                    <th className="text-left py-2 px-3 font-medium" style={{ color: '#030303' }}>Date</th>
                                    <th className="text-left py-2 px-3 font-medium" style={{ color: '#030303' }}>Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                {recentOrders.map(order => (
                                    <tr key={order.id} className="border-b" style={{ borderColor: '#E5E7EB' }}>
                                        <td className="py-2 px-3 font-mono" style={{ color: '#666666' }}>#{order.id.slice(-6)}</td>
                                        <td className="py-2 px-3" style={{ color: '#666666' }}>{new Date(order.orderDate).toLocaleDateString()}</td>
                                        <td className="py-2 px-3">
                                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(order.status)}`}>{order.status}</span>
                                        </td>
                                    </tr>
                        ...
                        <ReactRouterDOM.Link 
                            to="/caterer/orders" 
                            className="mt-4 block text-sm font-semibold text-right hover:underline"
                            style={{ color: '#123458' }}
                        >
                ...
                <div className="bg-white p-6 rounded-lg shadow-md border-2" style={{ borderColor: '#123458' }}>
                    <h2 className="text-xl font-bold mb-4" style={{ color: '#030303' }}>Top 5 Selling Items</h2>
                    {topSellingItems.length > 0 ? (
                        <ol className="space-y-2">
                            {topSellingItems.map((item, index) => (
                                <li key={index} className="flex justify-between items-center text-sm p-2 rounded-md even:bg-gray-50">
                                    <span className="font-medium" style={{ color: '#030303' }}>{index + 1}. {item.name}</span>
                                    <span className="font-bold" style={{ color: '#123458' }}>{item.count} units sold</span>
                                </li>
                            ))}
                        </ol>
                    ) : (
                        <div className="flex items-center justify-center h-full py-10" style={{ color: '#666666' }}>
                            No sales data available yet.
                        </div>
                    )}
                </div>
