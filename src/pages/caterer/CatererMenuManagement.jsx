...
const CatererMenuManagement = () => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold" style={{ color: '#030303' }}>My Menu Management</h1>
                <button 
                    onClick={() => openModal()} 
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full" style={{ backgroundColor: '#FFFFFF' }}>
                    <thead style={{ backgroundColor: '#F8FAFC' }}>
                        <tr>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Image</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Name</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Category</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Price</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Actions</th>
                        </tr>
                    </thead>
                    <tbody style={{ color: '#666666' }}>
                        {myMenuItems.map(item => (
                            <tr key={item.id} className="border-b align-middle" style={{ borderColor: '#E5E7EB' }}>
                                <td className="py-2 px-4">
                                    <img src={item.imageUrl} alt={item.name} className="w-16 h-16 object-cover rounded-md" />
                                </td>
                                <td className="py-3 px-4" style={{ color: '#030303' }}>{item.name}</td>
                                <td className="py-3 px-4" style={{ color: '#030303' }}>{categories.find(c => c.id === item.categoryId)?.name}</td>
                                <td className="py-3 px-4" style={{ color: '#030303' }}>PKR {item.price.toFixed(2)}</td>
                                <td className="py-3 px-4">
                                    <button 
...
const MenuItemModal = ({ item, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[60] p-4">
            <div className="rounded-lg shadow-xl w-full max-w-lg overflow-hidden border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
                <div className="p-4 border-b" style={{ backgroundColor: '#F8FAFC', borderColor: '#123458' }}>
                    <h2 className="text-xl font-bold text-center" style={{ color: '#030303' }}>{item ? 'Edit' : 'Add'} Menu Item</h2>
                </div>
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium" style={{ color: '#030303' }}>Name</label>
                            <input 
                                type="text" 
                                name="name" 
                                id="name" 
                        ...
                        <div>
                            <label htmlFor="description" className="block text-sm font-medium" style={{ color: '#030303' }}>Description</label>
                            <textarea 
                                name="description" 
                                id="description" 
                        ...
                        <div>
                            <label htmlFor="price" className="block text-sm font-medium" style={{ color: '#030303' }}>Price</label>
                            <input 
                                type="number" 
                                name="price" 
                                id="price" 
                        ...
                        <div>
                            <label htmlFor="imageUrl" className="block text-sm font-medium" style={{ color: '#030303' }}>Image</label>
                            <div className="mt-1 flex items-center">
                                {formData.imageUrl ? (
                                    <img src={formData.imageUrl} alt="Preview" className="w-24 h-24 object-cover rounded-md mr-4" />
                                ) : (
                                    <div className="w-24 h-24 rounded-md mr-4 flex items-center justify-center" style={{ backgroundColor: '#F3F4F6' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#9CA3AF' }}>
                                ...
                                <input 
                                    ...
                                    style={{ 
                                        color: '#666666',
                                        file: {
                                            backgroundColor: 'rgba(18, 52, 88, 0.1)',
                                            color: '#123458'
                                        }
                                    }}
                                />
                        ...
                        <div>
                            <label htmlFor="categoryId" className="block text-sm font-medium" style={{ color: '#030303' }}>Category</label>
                            <select 
                                name="categoryId" 
                                id="categoryId" 
