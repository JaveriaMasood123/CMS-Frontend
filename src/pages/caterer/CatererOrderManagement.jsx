...
//     const calculateMySubtotal = (order) => {
//         return order.items
...
const CatererOrderManagement = () => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold" style={{ color: '#030303' }}>My Orders</h1>
            </div>
            
            <div className="mb-6">
                <div className="relative max-w-sm">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#666666' }}>
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                    </div>
                    <input
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full" style={{ backgroundColor: '#FFFFFF' }}>
                    <thead style={{ backgroundColor: '#F8FAFC' }}>
                        <tr>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Order ID</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Customer</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Date</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: '#030303' }}>Status</th>
                    ...
                    <tbody style={{ color: '#666666' }}>
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map(order => (
                                <React.Fragment key={order.id}>
                                    <tr 
                                        className={`border-b hover:bg-gray-100 transition-colors ${expandedOrderId === order.id ? 'bg-blue-50' : ''}`}
                                        style={{ borderColor: '#E5E7EB' }}
                                    >
                                        <td className="py-3 px-4" onClick={() => toggleExpand(order.id)} style={{ color: '#030303' }}>#{order.id.slice(-6)}</td>
                                        <td className="py-3 px-4" onClick={() => toggleExpand(order.id)} style={{ color: '#030303' }}>{order.customerName}</td>
                                        <td className="py-3 px-4" onClick={() => toggleExpand(order.id)} style={{ color: '#030303' }}>{new Date(order.orderDate).toLocaleDateString()}</td>
                                        <td className="py-3 px-4" onClick={() => toggleExpand(order.id)}>
                                             <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(order.status)}`}>{order.status}</span>
                                        </td>
                                        <td className="py-3 px-4">
                                            <select 
                                                ...
                                                onChange={(e) => handleStatusChange(order.id, e.target.value)}
                                                onClick={(e) => e.stopPropagation()}
                                                className="border rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2"
                                                style={{ borderColor: '#123458', color: '#030303', backgroundColor: '#FFFFFF' }}
                                            >
                                                <option value="Pending">Pending</option>
                                                <option value="Confirmed">Confirmed</option>
                                    ...
                                    {expandedOrderId === order.id && (
                                        <tr style={{ backgroundColor: '#FFFFFF' }}>
                                            <td colSpan={6} className="p-4 border-b" style={{ borderColor: '#E5E7EB' }}>
                                                <div className="p-4 rounded-md border" style={{ backgroundColor: '#F8FAFC', borderColor: '#123458' }}>
                                                    <h4 className="font-bold mb-2" style={{ color: '#030303' }}>Your Items in this Order:</h4>
                                                    <ul className="divide-y" style={{ borderColor: '#E5E7EB' }}>
                                                        {order.items.filter(i => i.item.catererId === currentUser?.id).map(({ item, quantity }) => (
                                                            <li key={item.id} className="py-2 flex justify-between items-center">
                                                                <div>
                                                                    <span className="font-semibold" style={{ color: '#030303' }}>{item.name}</span>
                                                                    <span className="text-sm ml-2" style={{ color: '#666666' }}>({quantity} x PKR {item.price.toFixed(2)})</span>
                                                                </div>
                                                                <span className="font-semibold" style={{ color: '#030303' }}>PKR {(item.price * quantity).toFixed(2)}</span>
                                                            </li>
                                                        ))}
                                                    </ul>
                                                    <div className="flex justify-end font-bold text-lg mt-3 pt-3 border-t" style={{ borderColor: '#E5E7EB', color: '#030303' }}>
                                                        <span>Your Subtotal:</span>
                                                        <span className="ml-4">PKR {calculateMySubtotal(order).toFixed(2)}</span>
                                                    </div>
                                                </div>
