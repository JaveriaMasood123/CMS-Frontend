...
const OrdersPage = () => {
    ...
    return (
        <div className="p-8 rounded-lg shadow-lg border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
            <h1 className="text-3xl font-bold mb-6" style={{ color: '#030303' }}>Order History</h1>
            {userOrders.length === 0 ? (
                <p className="text-center py-8" style={{ color: '#666666' }}>You have not placed any orders yet.</p>
            ) : (
                <div className="space-y-6">
                    {userOrders.map(order => (
                        <div 
                        ...
                        >
                            <div className="flex justify-between items-start flex-wrap">
                                <div>
                                    <h2 className="text-xl font-semibold" style={{ color: '#123458' }}>Order #{order.id.slice(-6)}</h2>
                                    <p className="text-sm" style={{ color: '#666666' }}>Date: {new Date(order.orderDate).toLocaleDateString()}</p>
                                </div>
                                <div className="text-right">
                                    <span className={`px-3 py-1 text-sm font-semibold rounded-full ${getStatusColor(order.status)}`}>{order.status}</span>
                                    <p className="text-lg font-bold mt-2" style={{ color: '#030303' }}>Total: PKR {order.total.toFixed(2)}</p>
                                </div>
                            </div>
                            <div className="mt-4 border-t pt-4" style={{ borderColor: '#E5E7EB' }}>
                                <h3 className="font-semibold mb-2" style={{ color: '#030303' }}>Items:</h3>
                                <ul className="list-disc list-inside space-y-1" style={{ color: '#666666' }}>
                                    {order.items.map(({ item, quantity }) => (
                                        <li key={item.id}>{item.name} x {quantity}</li>
                                    ))}
                                </ul>
