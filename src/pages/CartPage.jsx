...
const CartPage = () => {
    ...
    return (
        <div className="p-8 rounded-lg shadow-lg border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
            <div className="flex justify-between items-center mb-6 flex-wrap">
                <h1 className="text-3xl font-bold" style={{ color: '#030303' }}>Your Cart</h1>
                {cart.length > 0 && (
                    <button 
            ...
            {cart.length === 0 ? (
                <div className="text-center py-10">
                    <p className="text-xl" style={{ color: '#666666' }}>Your cart is empty.</p>
                    <ReactRouterDOM.Link 
                        to="/menu" 
                        className="mt-4 inline-block px-6 py-2 rounded-md hover:opacity-90 transition-opacity font-semibold"
                ...
                <>
                        ...
                        {cart.map(({ item, quantity }) => (
                            <div key={item.id} className="flex items-center justify-between py-4 flex-wrap">
                                <div className="flex items-center space-x-4 mb-2 sm:mb-0">
                                    <img src={item.imageUrl} alt={item.name} className="w-20 h-20 rounded-md object-cover"/>
                                    <div>
                                        <h3 className="font-semibold" style={{ color: '#030303' }}>{item.name}</h3>
                                        <p style={{ color: '#666666' }}>PKR {item.price.toFixed(2)}</p>
                                    </div>
                                </div>
                                <div className="flex items-center space-x-4">
                                    <div className="flex items-center border rounded-md" style={{ borderColor: '#123458' }}>
                                        <button 
                                            ...
                                            className="px-3 py-1 text-lg font-bold hover:opacity-70 transition-opacity"
                                            style={{ color: '#123458' }}
                                        >-</button>
                                        <span className="px-4 py-1" style={{ color: '#030303' }}>{quantity}</span>
                                        <button 
                                            onClick={() => updateCartQuantity(item.id, quantity + 1)} 
                                            className="px-3 py-1 text-lg font-bold hover:opacity-70 transition-opacity"
                                            style={{ color: '#123458' }}
                                    ...
                                    <p className="font-semibold w-20 text-right" style={{ color: '#030303' }}>PKR {(item.price * quantity).toFixed(2)}</p>
                                    <button 
                    ...
                    <div className="mt-6 pt-6 border-t" style={{ borderColor: '#E5E7EB' }}>
                        <div className="flex justify-end items-center">
                            <span className="text-xl font-bold" style={{ color: '#030303' }}>Total:</span>
                            <span className="text-2xl font-bold ml-4" style={{ color: '#030303' }}>PKR {cartTotal.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-end mt-6 space-x-4">
                            <ReactRouterDOM.Link 
                                to="/menu" 
                                className="px-8 py-3 rounded-md font-semibold hover:opacity-90 transition-opacity border"
                            ...
                            <ReactRouterDOM.Link 
                                to="/checkout"
                                className="px-8 py-3 rounded-md font-semibold hover:opacity-90 transition-opacity"
                                style={{ backgroundColor: '#123458', color: 'white' }}
                            >
                                Proceed to Payment
                            </ReactRouterDOM.Link>
