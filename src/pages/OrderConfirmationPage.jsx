...
//     const { orderId } = ReactRouterDOM.useParams();
...
const OrderConfirmationPage = () => {
    ...
    if (!order) {
        return (
            <div className="text-center p-12 rounded-lg shadow-lg border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
                <h1 className="text-3xl font-bold mb-4" style={{ color: '#DC2626' }}>Order Not Found</h1>
                <p className="mb-8" style={{ color: '#666666' }}>We couldn't find the order you're looking for.</p>
                <ReactRouterDOM.Link 
                    to="/orders" 
    ...
    return (
        <div className="max-w-3xl mx-auto p-8 rounded-lg shadow-lg text-center border" style={{ backgroundColor: '#FFFFFF', borderColor: '#123458' }}>
            <svg className="w-16 h-16 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ color: '#059669' }}>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <h1 className="text-3xl font-bold mb-2" style={{ color: '#030303' }}>Thank You For Your Order!</h1>
            <p className="mb-6" style={{ color: '#666666' }}>Your order has been placed successfully and is now being processed.</p>

            <div className="p-6 rounded-lg text-left my-8 border" style={{ backgroundColor: '#F8FAFC', borderColor: '#123458' }}>
                <h2 className="text-xl font-semibold mb-4 border-b pb-2" style={{ color: '#030303', borderColor: '#123458' }}>Order Summary</h2>
                <div className="flex justify-between mb-2">
                    <span className="font-semibold" style={{ color: '#030303' }}>Order ID:</span>
                    <span style={{ color: '#666666' }}>#{order.id.slice(-6)}</span>
                </div>
                <div className="flex justify-between mb-4">
                    <span className="font-semibold" style={{ color: '#030303' }}>Order Date:</span>
                    <span style={{ color: '#666666' }}>{new Date(order.orderDate).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between mb-4">
                    <span className="font-semibold" style={{ color: '#030303' }}>Estimated Delivery:</span>
                    <span style={{ color: '#666666' }}>2-3 business days</span>
                </div>
                ...
                <div className="space-y-2 mb-4">
                    {order.items.map(({ item, quantity }) => (
                        <div key={item.id} className="flex justify-between items-center">
                            <div>
                                <p className="font-semibold" style={{ color: '#030303' }}>{item.name}</p>
                                <p className="text-sm" style={{ color: '#666666' }}>
                                    {quantity} x PKR {item.price.toFixed(2)}
                                </p>
                            </div>
                            <p className="font-semibold" style={{ color: '#030303' }}>PKR {(item.price * quantity).toFixed(2)}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="border-t pt-4 space-y-2" style={{ borderColor: '#E5E7EB' }}>
                    ...
                    <div className="flex justify-between font-bold text-xl mt-2" style={{ color: '#030303' }}>
                        <span>Total</span>
                        <span>PKR {(order.total + 250).toFixed(2)}</span>
                    </div>
                </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
                <ReactRouterDOM.Link 
                    to="/orders" 
                    className="px-6 py-3 rounded-md hover:opacity-90 transition-opacity font-semibold"
                    style={{ backgroundColor: '#123458', color: 'white' }}
                >
                    View Order History
                </ReactRouterDOM.Link>
                <ReactRouterDOM.Link 
                    to="/menu" 
                    className="px-6 py-3 rounded-md hover:opacity-90 transition-opacity font-semibold border"
