...
const MenuItemCard = ({ item }) => {
    ...
    return (
        <ReactRouterDOM.Link 
            to={`/menu/${item.id}`} 
            className="block bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 group flex flex-col border border-gray-200"
        >
            <div className="relative">
                <img className="w-full h-48 object-cover" src={item.imageUrl} alt={item.name} />
                {currentUser?.role === 'CUSTOMER' && (
                    <button 
                        ...
                        className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm rounded-full p-2 transition-transform hover:scale-110" 
                ...
                <button 
                    ...
                    className="absolute bottom-2 right-2 text-white rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity hover:opacity-90 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-white" 
            ...
            <div className="p-6 flex flex-col flex-grow">
                <p className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: '#666666' }}>
                    {caterer?.businessName || 'GourmetGo'}
                </p>
                <h3 className="text-xl font-semibold mb-2 group-hover:underline transition-colors flex-grow" style={{ color: '#030303' }}>
                    {item.name}
                </h3>
                <p className="mb-4 h-12 overflow-hidden text-sm" style={{ color: '#666666' }}>
                    {item.description.substring(0, 70)}...
                </p>
                <div className="flex justify-between items-center mt-auto">
                    <span className="text-2xl font-bold" style={{ color: '#123458' }}>
                        PKR {item.price.toFixed(2)}
                    </span>
                    <span className="font-semibold text-sm" style={{ color: '#123458' }}>
                        View Details &rarr;
                    </span>
                </div>
            </div>
